...
def _check_quota(
    ...
    window_end = (
        now +
        timedelta(
            hours=QUOTA_WINDOW_HOURS
        )
    )


    if not (
        now < start <= window_end
    ):
        return
...
def create_booking(
    ...
    start = parse_input_datetime(
        payload.start_time
    )

    end = parse_input_datetime(
        payload.end_time
    )
    ...
    # Must be future, no grace period
    if start <= now:
    ...
    if (
        ...
        or duration_hours > MAX_DURATION_HOURS
    ):
...
def cancel_booking(
    ...
    notifications.notify_cancelled(
        booking
    )
